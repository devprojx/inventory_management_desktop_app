	...
	public partial class ReceivedLog : Form
	{
		...
		void FindTransactionByDate(string date)
		{
			try
			{
				...
				string sql  = "SELECT ItemTbl.ItemName, SupplierTbl.SupplierName, ReceivedLogTbl.DateReceived, ReceivedLogTbl.QuantityReceived, ReceivedLogTbl.Cost, SupplierTbl.SupplierName FROM (ReceivedLogTbl " +
					                        ...
					                        " Where  ReceivedLogTbl.DateReceived like [0]";
		...
		void FindTransactionByItemName(string itemname)
		{
			...
			{
				accessConnection.Open();
				string sql  = "SELECT ItemTbl.ItemName, SupplierTbl.SupplierName, ReceivedLogTbl.DateReceived, ReceivedLogTbl.QuantityReceived, ReceivedLogTbl.Cost FROM (ReceivedLogTbl " +
					                        "INNER JOIN  ItemTbl ON ReceivedLogTbl.ItemCode = ItemTbl.ItemCode) INNER JOIN SupplierTbl ON ReceivedLogTbl.SupplierId = SupplierTbl.SupplierId" +
					                        " Where ItemTbl.ItemName like [0]";
				OleDbCommand cmd = new OleDbCommand();
				cmd.Connection = accessConnection;
				cmd.CommandText = sql;
				itemname = itemname + "%";
		...
		void FindTransactionBySupplier(string supplier)
		{
			try
			{
				accessConnection.Open();
				string sql  = "SELECT ItemTbl.ItemName,SupplierTbl.SupplierName, ReceivedLogTbl.DateReceived, ReceivedLogTbl.QuantityReceived, ReceivedLogTbl.Cost FROM (ReceivedLogTbl " +
					                        "INNER JOIN  ItemTbl ON ReceivedLogTbl.ItemCode = ItemTbl.ItemCode) INNER JOIN SupplierTbl ON ReceivedLogTbl.SupplierId = SupplierTbl.SupplierId" +
					                        " Where SupplierTbl.SupplierName like [1]";
				OleDbCommand cmd = new OleDbCommand();
				cmd.Connection = accessConnection;
				cmd.CommandText = sql;
				supplier = supplier + "%";
		...
		void FindTransactionByItemNameAndDate(string itemname, string date)
		{
			try
			{
				accessConnection.Open();
				string sql  = "SELECT ItemTbl.ItemName, SupplierTbl.SupplierName, ReceivedLogTbl.DateReceived, ReceivedLogTbl.QuantityReceived, ReceivedLogTbl.Cost FROM (ReceivedLogTbl " +
					                        "INNER JOIN  ItemTbl ON ReceivedLogTbl.ItemCode = ItemTbl.ItemCode) INNER JOIN SupplierTbl ON ReceivedLogTbl.SupplierId = SupplierTbl.SupplierId" +
					                        " Where ItemTbl.ItemName like [0] AND ReceivedLogTbl.DateReceived like [1]";
				OleDbCommand cmd = new OleDbCommand();
				cmd.Connection = accessConnection;
				cmd.CommandText = sql;
				itemname = itemname + "%";
		...
		void FindTransactionBySupplierAndDate(string supplier, string date)
		{
			try
			{
				accessConnection.Open();
				string sql  = "SELECT ItemTbl.ItemName, SupplierTbl.SupplierName, ReceivedLogTbl.DateReceived, ReceivedLogTbl.QuantityReceived, ReceivedLogTbl.Cost FROM (ReceivedLogTbl " +
					                        "INNER JOIN  ItemTbl ON ReceivedLogTbl.ItemCode = ItemTbl.ItemCode) INNER JOIN SupplierTbl ON ReceivedLogTbl.SupplierId = SupplierTbl.SupplierId" +
					                        " Where SupplierTbl.SupplierName like [0] AND ReceivedLogTbl.DateReceived like [1]";
				OleDbCommand cmd = new OleDbCommand();
				cmd.Connection = accessConnection;
				cmd.CommandText = sql;
				supplier = supplier + "%";
		...
		void FindTransactionByItemNameSupplierAndDate(string itemname, string date, string supplier)
		{
			try
			{
				...
				string sql  = "SELECT ItemTbl.ItemName, SupplierTbl.SupplierName, ReceivedLogTbl.DateReceived, ReceivedLogTbl.QuantityReceived, ReceivedLogTbl.Cost FROM (ReceivedLogTbl " +
					                        "INNER JOIN  ItemTbl ON ReceivedLogTbl.ItemCode = ItemTbl.ItemCode) INNER JOIN SupplierTbl ON ReceivedLogTbl.SupplierId = SupplierTbl.SupplierId" +
					                        " Where ItemTbl.ItemName like [0] AND ReceivedLogTbl.DateReceived like [1] AND SupplierTbl.SupplierName like [2]";
				OleDbCommand cmd = new OleDbCommand();
				cmd.Connection = accessConnection;
				cmd.CommandText = sql;
				itemname = itemname + "%";
		...
		{
			if(itemnameTxtBox.Text != "" && itemnameTxtBox.Text != " ")
			{
				FindTransactionByItemName(itemnameTxtBox.Text);
			}
			else if(dateMskTxtbx.MaskCompleted)
			{
				FindTransactionByDate(dateMskTxtbx.Text);
			}
			else if(supplierTxtbox.Text != "" && supplierTxtbox.Text != " ")
			{
				FindTransactionBySupplier(supplierTxtbox.Text);
			}
			else if(itemnameTxtBox.Text != "" && itemnameTxtBox.Text != " " && dateMskTxtbx.MaskCompleted)
			{
				FindTransactionByItemNameAndDate(itemnameTxtBox.Text, dateMskTxtbx.Text);
			}
			else if(supplierTxtbox.Text != "" && supplierTxtbox.Text != " " && dateMskTxtbx.MaskCompleted)
			{
				FindTransactionBySupplierAndDate(supplierTxtbox.Text, dateMskTxtbx.Text);
			}
			else if(itemnameTxtBox.Text != "" && itemnameTxtBox.Text != " " && dateMskTxtbx.MaskCompleted
			        && supplierTxtbox.Text != "" && supplierTxtbox.Text != " ")
			{
