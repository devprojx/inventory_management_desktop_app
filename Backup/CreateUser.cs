...
namespace Inventory_System
{
	...
	public partial class CreateUser : Form
	{
		...
		void CreateBtnClick(object sender, EventArgs e)
		{
			...
			if(ValidatePassword.getInstance().CheckIfUserExist(newUser.Username))
			{
		...
		public void CreateNewUser(User newUser)
		{
			...
			try
			{
				...
				cmd.CommandText = @"INSERT INTO LoginTbl(Username, UserPassword, SecretQuestion, Answer)" +
					               "VALUES([0], [1], [2], [3])";
				cmd.Parameters.AddWithValue("0", newUser.Username.ToLower());
				cmd.Parameters.AddWithValue("0",newUser.Password);
				cmd.Parameters.AddWithValue("0", newUser.SecretQuestion);
				cmd.Parameters.AddWithValue("0", newUser.Answer.ToLower());
				cmd.ExecuteNonQuery();
		...
		void ChangeBtnClick(object sender, EventArgs e)
		{
			...
			if(!(ValidatePassword.getInstance().ValidateUser(username2Txtbox.Text, tempPassword)))
			{
			...
			ValidatePassword.getInstance().Clear(groupBox2);
		}
