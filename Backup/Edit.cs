	...
	public partial class Edit : Form
	{
			...
			if(IsInfoEmpty())
			{
			 ...
			 errLb2.Visible = false;
	    	   ...
	    	   try
	    	   {
					...
					cmd.Connection = accessConnection;
				  	cmd.CommandText =  "UPDATE ItemTbl SET  ItemName = [1], ItemDescription = [2], StatusId = [3] WHERE ItemCode = [4]";
				    ...
				    cmd.Parameters.AddWithValue("4", itemCode);
				    cmd.ExecuteNonQuery();
		...
		void refreshList()
		{
			...
			try
			{
				...
				cmd = new OleDbCommand();
				cmd.Connection =  accessConnection;
				cmd.CommandText = @"SELECT ItemCode, ItemName, ItemDescription, ItemQuantity,ReorderLevel,Unit, StatusTbl.StatusId From ItemTbl 
                                     INNER JOIN StatusTbl ON ItemTbl.StatusId = StatusTbl.StatusId where itemName = [0]";
